...
def process_parquet_files(directory_path, batch_size=200):
    ...
    with multiprocessing.Pool() as pool:
        ...
        for i, (file_path, error) in enumerate(tqdm(pool.imap_unordered(batch_insert_parquet_to_postgres, args_list),
            ...
            if (i + 1) % 10 == 0 or i == len(parquet_files) - 1:
                logging.info(f"Processed {i+1}/{len(parquet_files)} files. Successful: {len(successful)}, Failed: {len(failed)}")
