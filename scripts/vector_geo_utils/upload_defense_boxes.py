...
def batch_insert_parquet_to_postgres(args):
    ...
    try:
        ...
        df = df[["geometry", "embedding", "chip_id"]]
        ...
        for _, row in df.iterrows():
            ...
            try:
                data_to_insert.append((wkt_geometry, embedding.tolist(), chip_id))
            ...
            if len(data_to_insert) >= batch_size:
                with psycopg2.connect(postgres_uri) as conn:
                    with conn.cursor() as cur:
                        ...
                        query = """
                        INSERT INTO defense_boxes (location, embedding, chip_id)
                        VALUES (ST_GeomFromText(%s, 4326)::geography, %s, %s)
